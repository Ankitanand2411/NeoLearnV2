...
class FakePersistence:
    ...
    async def persist_completion(self, user_id, topic_id, mastery):
        self.completions.append({"user_id": user_id, "topic_id": topic_id, "mastery": mastery})
        return {"badges_awarded": ["First Steps", "Expert"] if mastery >= 0.7 else ["First Steps"], "completed_topics": 1}
...
def install_fakes(monkeypatch, *, replies=None, judge_error: Exception | None = None, question_error: Exception | None = None):
    ...
    monkeypatch.setattr(ai_service, "get_topic_context", lambda topic, topic_id=None: {
        "description": "d", "explanation": "e", "key_takeaway": "k", "mentor_id": "einstein", "id": "topic-1",
    })

    judge_calls = {"n": 0}

    async def evaluate_understanding(topic, history, persona_id=None, topic_id=None):
        judge_calls["n"] += 1
    ...
    monkeypatch.setattr(persistence, "persist_completion", fake_persistence.persist_completion)
